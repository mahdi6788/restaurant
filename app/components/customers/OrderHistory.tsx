        ...
        {orders.map((order: Order & { items: OrderItemWithmenuItem[] }) => (
          <div key={order.id} className="mb-2 rounded-lg bg-gray-50 dark:dark-mode p-1 sm:p-4">
            ...
            <table className="table sm:hidden w-full max-w-[100%] table-fixed border-collapse border border-gray-300 mb-10">
              <thead className="rounded-lg text-left text-sm">
                <tr>
                  <th className="px-1 py-1 border-2 font-medium">{translate("Name")}</th>
                  <th className="px-1 py-1 border-2 font-medium text-center">{translate("Price")}</th>
                  <th className="px-1 py-1 border-2 font-medium text-center w-[10%]">{translate("Qty")}</th>
                  <th className="px-1 py-1 border-2 font-medium text-center">{translate("Date")}</th>
                  <th className="px-1 py-1 border-2 font-medium text-center">{translate("Payment")}</th>
                </tr>
              </thead>
              <tbody className="">
                {order.items.map((orderItem: OrderItemWithmenuItem) => (
                  ...
                  >
                    <td className="flex items-center gap-1 whitespace-normal py-1 px-2">
                      <button
                        disabled={!orderItem.menuItem.isAvailable}
                        className=""
                        onClick={() =>
                    ...
                    <td className="whitespace-normal py-1 px-2 text-center ">
                      AED {orderItem.price.toFixed(2)}
                    </td>
                    <td className="whitespace-normal py-1 px-2 text-center w-[10%]">
                      {orderItem.quantity}
                    </td>
                    <td className="whitespace-normal py-1 px-2 text-center ">
                      {new Date(order.createdAt).toLocaleDateString()}
